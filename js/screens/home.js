...
function buildHeroSection({ examCountdown, todayPlan, streak, streakBadge, overallPct, srsSummary }) {
  ...
  titleGroup.appendChild(createElement('div', { classes: ['hero-app-label'], text: 'FE EXAM' }));
  titleGroup.appendChild(createElement('h1', { classes: ['hero-app-title'], text: 'FE ナビ' }));
...
function buildOnboardingCard() {
  ...
  // 「教科書から始める」ボタン → 科目A の一覧へ遷移
  // 詳細な節IDが定まったら page クエリで直接遷移するように更新する
