...
/** 模擬試験の科目別出題数定義（FE 科目A：60問構成・科目Bは別建て予定） */
// TODO(Phase 2+): 科目B（20問・100分）の模擬試験は別フローで実装する
const EXAM_QUESTION_COUNTS = {
  A: 60,
};

/** 模擬試験の制限時間（秒）：科目A 90分 = 5400秒 */
const EXAM_TIME_LIMIT_SEC = 90 * 60;
...
function openModeSettingsModal({ container, mode, initialCategory, initialCount }) {
  ...
  if (mode.id === 'exam') {
    ...
    info.appendChild(createElement('p', {
      ...
      text: '科目A：60問・90分の本番形式で出題されます（テクノロジ系・マネジメント系・ストラテジ系の総合問題）。',
    }));
    ...
    // 下段：科目A / 科目B を均等2カラムで並べる（FE は2科目構成）
...
const PAST_YEAR_OPTIONS = [
  // FE は CBT 化（2023 年4月）以降、IPA が年度ごとに公開問題を発表
  // 令和7年度 科目A 20問が公開問題として取り込み済み（科目B は将来対応）
  { source: 'past_R07_FE_A', label: '令和7年度 公開問題（科目A）', count: 20 },
];
...
function renderExplanationScreen(container) {
  ...
  if (current.related_page_id) {
    ...
    reviewBtn.addEventListener('click', () => {
      ...
      // related_page_idのプレフィックスから科目を判定する（FE: A 系 / B 系）
      // 例 A-tech-01-01 → A、B-prog-03-02 → B
      const prefix = current.related_page_id.split('-')[0];
      const sectionId = (prefix === 'A' || prefix === 'B') ? prefix : 'A';
